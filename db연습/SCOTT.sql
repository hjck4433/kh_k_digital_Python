...
-- 중복 제거하기 : DISTINCT, 데이터를 조회할 때 값이 중복되는 행이 여러개 조회되는 경우, 값이 중복된 행을 하개씩만 선택하고자하는 경우 사용
...
-- 같이 않음을 표현하는 방법은 여러가지 존재 <>, != , ^=, NOT
SELECT * FROM EMP
WHERE NOT DEPTNO = 30;

-- 급여가 3000이상이고 부서가 20번인 사원 조회 (2가지 조건을 모두 만족해야 하는 경우_
SELECT * 
FROM EMP
...
-- 급여가 3000이상이고 부서가 20번이거나 입사일이 82년 1월1일 이전인 사원 조회 
SELECT *
FROM EMP
WHERE SAL >=3000 AND (DEPTNO = 20 OR HIREDATE <'82/01/01');

-- 문제 1 : 급여가 2500이상이고 직업이 MANAGER인 사원정보만 출력
SELECT *
FROM EMP
...
-- %P를 검색하고 싶으면 LIKE '%\%P' ESCAPE '\'; 사용

-- NULL : 미확정 또는 알 수 없는 값을 의미함, 그래서 연산, 할당, 비교 불가 (=, IN)
SELECT ENAME, SAL*12+COMM AS 연봉, COMM
FROM EMP;

-- IS NULL / IS NOT NULL : NULL 여부를 확인할 때 사용하는 연산자
SELECT *
FROM EMP
WHERE COMM IS NOT NULL; -- = NULL로 하면 연산 불가
