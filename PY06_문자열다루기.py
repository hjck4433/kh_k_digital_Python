...
b = a[0] + a[1] + a[2] + a[3]
print(b)
# a[1] = "L" 문자열의 요소는 읽을 수는 있지만 변경은 되지 않음. literal 상수

# 대소문자 바꾸기
aa = "Hello Python Program...."
print(aa.upper())  # 기존 값(aa)이 변한 것은 아님
bb = aa.upper()
print(bb)
print(aa.lower())
...
print(len(input_str2))  # 문자열의 길이를 반환, 별도의 외부 함수를 사용하는 방식 / len() 외부 함수
...
print(phrase.rfind("가장"))  # 34 뒤에서부터 찾지만 위치에 대한 인덱스는 앞에서부터 계산
print(phrase.index("포기"))
print(phrase.rindex("가장"))

print(phrase.find("나에게"))  # 못 찾으면 -1
# print(phrase.index("나에게"))  # 못 찾으면 value error

new_phrase = phrase.replace("가장", "나에게",2)  # count에 값 넣으주면 해당 개수 만큼 바뀜
print(new_phrase)
